...
def main():
    ...
    choice = int(input("What would you to do with the program? \n\
    Add a character(1)\n\
...
def remove_character():
    ...
    cur.execute("DELETE from characters WHERE name = (?)", (character_name,))
...
def display_certain_characters():
    ...
    if sort_type == 1:
        ...
        cur.execute("SELECT * FROM characters WHERE age >= (?)", (age_choice,))
        characters = cur.fetchall()
    ...
    #Sorting by the rank
        ...
        cur.execute("SELECT * FROM characters WHERE rank >= (?)", (rank_choice,))
        characters = cur.fetchall()
